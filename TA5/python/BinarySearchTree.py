...
class Tree(object):
    ...
    def __iter__(self):
        keep = []
        self.inorder(keep)
        return keep.__iter__()  # List iterator

    def create_custom_iterator(self, iterator_type: int):
        keep = []
        if iterator_type == 0:  # Inorder
            self.inorder(keep)
        elif iterator_type == 1:  # Preorder
            self.preorder(keep)
        elif iterator_type == 2:  # Postorder
            self.postorder(keep)
        elif iterator_type == 3:  # Levelorder
            self.level_order(keep)
        else:
            self.level_order(keep)  # ReverseLevelOrder
            keep.reverse()
...
if __name__ == '__main__':
    ...
    it1 = tree.create_custom_iterator(0)
    it2 = tree.create_custom_iterator(1)
    it3 = tree.create_custom_iterator(2)
    it4 = tree.create_custom_iterator(3)
